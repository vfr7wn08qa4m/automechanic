...
class AdoClient:
    ...
    def ring_quota_mark(self, hours: float | None = None) -> None:
        ...
        ДЛИТЕЛЬНОСТЬ. Сначала стояло 5 часов — по сроку суточного сброса Gemini.
        Оказалось, ставка слишком крупная: метку выставляет ОДИН неудачный
        healthcheck, а он падает не только от исчерпанной квоты, но и от
        минутного всплеска rate-limit сразу по всем ключам. 2026-08-06 ровно
        одна такая осечка выключила дистилляцию во всём кольце: 63 тика из 86
        напечатали «квота выбрана» и ушли к производителям, выработка упала с
        413 кейсов до 33, а очередь subs за те же сутки выросла на 2509.
        Держим короткое окно: если квота и правда мертва, следующий тик просто
        поставит метку заново — это один дешёвый ping раз в QUOTA_COOLDOWN_H
        вместо потерянных полудня.
        """
        import os as _os
        import time as _time
        if hours is None:
            hours = float(_os.getenv("QUOTA_COOLDOWN_H", "0.75"))
        wid = self._ring_lock_item(create=True)
        if not wid:
            return
        try:
